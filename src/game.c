...
bool game(char secret_sequence[], char user_sequence[]){
    ...
    while (score_ou_jouer != 'a' && score_ou_jouer != 'b' && score_ou_jouer != 'q')
    {
            
        score_ou_jouer = tolower(saisir_caractere());
        if(score_ou_jouer != 'a' && score_ou_jouer != 'b' && score_ou_jouer != 'q')
            print_color_text("\nVeuillez saisir a, b ou q\n", 'R');
    }
